...
class VideoDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        episode = int(config["Dataset"].get("episode_index", 0))
        video_key = config["Dataset"].get("video_key", "observation.images.wrist")
        chunk = int(config["Dataset"].get("episode_chunk", 0))
        video_path = config["Dataset"].get("video_path")
        ...
        self.num_imgs = min(frame_count, int(max_frames)) if max_frames else frame_count

        self.color_paths = []
        self.depth_paths = []
        frame_dir = Path(dataset_path) / "images" / video_key / f"episode_{episode:06d}"
        depth_key = config["Dataset"].get("depth_key")
        depth_dir = (
            Path(dataset_path) / "images" / depth_key / f"episode_{episode:06d}"
            if depth_key
            else None
        )
        for idx in range(self.num_imgs):
            self.color_paths.append(str(frame_dir / f"frame_{idx:06d}.png"))
            if depth_dir is not None:
                self.depth_paths.append(str(depth_dir / f"frame_{idx:06d}.png"))
        self.poses = [np.eye(4, dtype=np.float32) for _ in range(self.num_imgs)]

    def __getitem__(self, idx):
        if idx < 0 or idx >= self.num_imgs:
