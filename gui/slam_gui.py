...
class SLAM_GUI:
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.quality_metrics is not None:
            self.quality_metrics = gaussian_packet.quality_metrics
    ...
    def overlay_quality_metrics(self, image):
        if self.quality_metrics is None:
            return image

        frame_idx = self.quality_metrics.get("frame_idx", -1)
        lines = [
            f"frame {frame_idx}",
            "q_norm "
            f"cov {self.quality_metrics.get('q_cov_norm', 0.0):.3f}  "
            f"unc {self.quality_metrics.get('q_unc_norm', 0.0):.3f}  "
            f"res {self.quality_metrics.get('q_res_norm', 0.0):.3f}",
            "raw    "
            f"cov {self.quality_metrics.get('q_cov', 0.0):.3f}  "
            f"unc {self.quality_metrics.get('q_unc', 0.0):.3f}  "
            f"res {self.quality_metrics.get('q_res', 0.0):.3f}",
        ]
    ...
    def get_current_cam(self):
        ...
        image_height = int(self.window.size.height)
        image_width = int(self.widget3d_width)
        vfov_deg = self.widget3d.scene.camera.get_field_of_view()
        hfov_deg = self.vfov_to_hfov(vfov_deg, image_height, image_width)
    ...
    def render_o3d_image(self, results, current_cam):
        if self.depth_chbox.checked:
            ...
            depth = torch.permute(depth, (2, 0, 1)).float()
            depth = (depth).byte().permute(1, 2, 0).contiguous().cpu().numpy()
            depth = self.overlay_quality_metrics(depth)
            ...
            opacity = self.overlay_quality_metrics(opacity)
            ...
            cv2.flip(img, 0, img)
            img = self.overlay_quality_metrics(img)
            ...
            rgb = self.overlay_quality_metrics(rgb)
